...
void bfs_single_thread(Graph graph, int nodes_count, function func) {
  vector<int> color;
...
double bfs(Graph graph, int nodes_count, function func) {
...
int main(int argc, char* argv[]) {
  ...
  __ROOT
  init_graph(&graph, nodes_count, 0.65);
  ...
  for (int i = 0; i < 100; i++) {
    ...
    if (processes_number == 1) {
      bfs_single_thread(graph, nodes_count, test);
    } else {
      summ_root_time += bfs(graph, nodes_count, test);
    }
